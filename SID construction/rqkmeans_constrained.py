...
def balanced_kmeans_level_constrained(X, K, max_iter=100, tol=1e-7, random_state=None, verbose=False):
    ...
    每层的 256 个 code 不希望只集中被少数 code 使用；约束每个 cluster 的样本量接近均衡。"""
    start_time = time.time()
    n, d = X.shape
    X = X.astype(np.float32, copy=False)

    # Calculate min and max cluster size，用来约束簇的大小
...
def residual_kmeans_constrained(X, K, L, max_iter=300, tol=1e-4, random_state=None, verbose=False):
    ...
    第1层拟合原始embedding，后续层拟合前一层重构后剩下的residual，因此一个商品得到L层离散code（训练多个codebook）
    ...
    Returns:
        codes_all: (L, N) integer codes for each level；每一行表示一层codebook，每一列表示一个商品；转置后得到我们需要的所有商品的dif
        codebooks: List of L codebooks, each (K, d)；一个列表，每层保存一个聚类中心矩阵；一共训练3 codebooks
...
def deal_with_deduplicate(df):
    ...
    原始三层 code 可能发生完整路径碰撞；脚本为冲突项追加额外 token，保证最终 index.json 中每个商品 SID 唯一"""
...
def analyze_codes(codes, title="", verbose=True):
    ...
    统计：每层用了多少个不同 code；完整路径有多少种；collision rate(1-所有路径/所有商品) 是多少"""
    N, M = codes.shape
    if verbose:
        if title:
            print(f"\n{title}")
